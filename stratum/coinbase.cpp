...
//	debuglog("pack tx %s", data+ol);
//	debuglog("pack tx %lld", amount);
}
...
//	debuglog("aux_script is %s", aux_script);
}
...
//	char script2[32] = "7969696d7000"; // "yiimp\0" in hex ascii
	char script2[64] = "166d747020666f72207969696d7020627920646a6d3334"; // "mtp for yiimp by djm34" in hex ascii
	if(!coind->pos && !coind->isaux && templ->auxs_size)
		coinbase_aux(templ, script2);

	int script_len = strlen(script1)/2 + strlen(script2)/2 + 8;

	if ( (strcmp(coind->symbol, "TCR_TESTNET") == 0) || (strcmp(coind->symbol, "TCR_MTP") == 0) || (strcmp(coind->symbol, "TCR_MTP") == 0) || (strcmp(coind->symbol, "XZC") == 0))
		script_len += 4; // size 8 for extranonce2
	...
	sprintf(templ->coinb2, "%sffffffff", script2);
	...
	else if(strcmp(coind->symbol, "DYN") == 0)
	{
		...
		if(!dynode && json_get_bool(json_result, "dynode_payments")) {
			...
			debuglog("%s is using old dynodes rpc keys", coind->symbol);
			return;
		}
		...
		if(superblocks_enabled && superblock) {
			for(int i = 0; i < superblock->u.array.length; i++) {
				...
				if (payee && amount) {
					...
					//debuglog("%s superblock found %s %u", coind->symbol, payee, amount);
				}
			}
		}
		if (dynode_enabled && dynode) {
			...
			if (payee && amount && started) {
				...
				base58_decode(payee, script_payee);
				job_pack_tx(coind, script_dests, amount, script_payee);
				//debuglog("%s dynode found %s %u", coind->symbol, payee, amount);
			}
		}
		...
		//debuglog("%s %d dests %s", coind->symbol, npayees, script_dests);
		return;
	}
	else if(strcmp(coind->symbol, "TCR") == 0 || strcmp(coind->symbol, "TCR_MTP") == 0) {
		const bool isMainnet = false;
		char script_payee[1024];

		coind->charity_percent = 0; 
		bool hasMasternode = false;

		/**
         * coinbasevalue - miners reward with fees
         * coinbasesubsidy - total block subsidy(including miner reward) without fees
		 * */
		const int64_t blockSubsidy       =                  json_get_int(json_result, "coinbasesubsidy");
		...
		bool masternode_payments_started = true;
		json_value* masternode = json_get_object(json_result, "tnode");
		const char *masternode_payee;
		const char * devTeam = "TC4frBMpSm2PF2FuUNqJ3qicn4EHL59ejL";
		const char * scienceProjects = "TNTkzXXJf8Yw3W1i29iQQgcxVfc3JicS2s";
		const char * cryptoInterest = "TD6A1JC3jUT91riUxpQpMQZJVBa4xU2vQC";
		json_int_t masternode_amount;

		if (masternode_payments_started && masternode) {
			bool started = true;
			masternode_payee = json_get_string(masternode, "payee");
			masternode_amount = json_get_int(masternode, "amount");
			hasMasternode = masternode_payee && masternode_amount && started;
		}
		if(hasMasternode){
		...
		if(templ->height == 1){
			// do nothing...
		} else if(templ->height < 600){
            job_pack_tx(coind, templ->coinb2, available, NULL);
            base58_decode(devTeam, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);
            base58_decode(scienceProjects, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 79 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);

		} else if(templ->height < rewardsStage2Start){
		    // T1
            job_pack_tx(coind, templ->coinb2, available, NULL);
            base58_decode(devTeam, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);
            base58_decode(scienceProjects, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 40 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);

		} else if(templ->height < rewardsStage3Start){
		    // T2
            job_pack_tx(coind, templ->coinb2, available, NULL);
            base58_decode(devTeam, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);
            base58_decode(scienceProjects, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 64 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);

		} else if(templ->height < rewardsStage4Start){
		    // T3
            job_pack_tx(coind, templ->coinb2, available, NULL);
            base58_decode(devTeam, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);
            base58_decode(scienceProjects, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 55 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 15 / 100, script_payee);
		} else if(templ->height < rewardsStage5Start){
		    // T4
            job_pack_tx(coind, templ->coinb2, available, NULL);
            base58_decode(devTeam, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);
            ...
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 40 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 20 / 100, script_payee);
		} else if(templ->height < rewardsStage6Start){
		    // T5
            job_pack_tx(coind, templ->coinb2, available, NULL);
            base58_decode(devTeam, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 10 / 100, script_payee);
            base58_decode(scienceProjects, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 35 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 20 / 100, script_payee);
		} else {
		    // T6
            ...
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 20 / 100, script_payee);
            base58_decode(cryptoInterest, script_payee);
            job_pack_tx(coind, templ->coinb2, blockSubsidy * 25 / 100, script_payee);
		}

		//Masternodes payments in the end
		if(hasMasternode){
			base58_decode(masternode_payee, script_payee);
			debuglog("TCR will pay masternode %s", masternode_payee);
			bool masternode_use_p2sh = (strcmp(coind->symbol, "MAC") == 0);
			if(masternode_use_p2sh)
				p2sh_pack_tx(coind, templ->coinb2, masternode_amount, script_payee);
			else
				job_pack_tx(coind, templ->coinb2, masternode_amount, script_payee);
		}
	...
	else if(strcmp(coind->symbol, "TCR_TESTNET") == 0) {
		char script_payee[1024];
		...
		json_value* masternode = json_get_object(json_result, "tnode");
		const char *masternode_payee;
		const char * devTeam = "GcbaqmHPjnB4JfDAKVhJb3X7g1kCf2e9Pm";
		const char * scienceProjects = "GXQqws2DWjUydoktzCrFAz3fJG177usYY3";
		const char * cryptoInterest = "GewZWTVEurJ2eSN4tfWrwDuqwpxkcRSbsR";
		json_int_t masternode_amount;
	...
	else if(strcmp(coind->symbol, "TCR_REGTEST") == 0) {
		char script_payee[1024];
		 ...
		 * */
		const int64_t blockSubsidy   = json_get_int(json_result, "coinbasesubsidy");
		const int rewardsStage2Start = 3;
		const int rewardsStage3Start = 10;
		const int rewardsStage4Start = 200000;
		const int rewardsStage5Start = 300000;
		const int rewardsStage6Start = 400000;


		bool masternode_payments_started = true;
		json_value* masternode = json_get_object(json_result, "tnode");
		const char *masternode_payee;
		const char * devTeam = "fdAAqxKZrgvt6o4YKSer2rNWaw1y5C8vZU";
		const char * scienceProjects = "fc52erY95gPPodezPd3Rfj6h4DWKPN8pkM";
		const char * cryptoInterest = "fT5R28XFJx7A618pNVjBoQmyYNQviqCzBs";
		json_int_t masternode_amount;
	...
	if(coind->charity_percent && !coind->hasmasternodes)
	{
		...
		coind->reward = (double)available/100000000*coind->reward_mul;
		//debuglog("INFO %s block available %f, charity %f miner %f", coind->symbol,
	...
	if(coind->hasmasternodes && !coind->oldmasternodes)
	{
		...
		if(!masternode && json_get_bool(json_result, "masternode_payments")) {
			coind->oldmasternodes = true;
			debuglog("%s is using old masternodes rpc keys", coind->symbol);
			return;
		}
			...
			for(int i = 0; i < superblock->u.array.length; i++) {
				...
				if (payee && amount) {
					...
					if(superblock_use_p2sh)
						...
						job_pack_tx(coind, script_dests, amount, script_payee);
					//debuglog("%s superblock %s %u", coind->symbol, payee, amount);
				}
			}
		}
		...
		strcat(templ->coinb2, "00000000"); // locktime
		coind->reward = (double)available/100000000*coind->reward_mul;
		//debuglog("%s total %u available %u", coind->symbol, templ->value, available);
		//debuglog("%s %d dests %s", coind->symbol, npayees, script_dests);
		return;
	}
		...
		if(strcmp(coind->symbol, "CRW") == 0)
		{
			...
			if(systemnodes_enabled && systemnodes) {
				...
				if (payeeSN && payeeSN_amount) {
					...
					//debuglog("%s systemnode %s %u", coind->symbol, payeeSN, payeeSN_amount);
				}
			}
	...
	coind->reward = (double)available/100000000*coind->reward_mul;
//	debuglog("coinbase %f", coind->reward);

//	debuglog("coinbase %s: version %s, nbits %s, time %s", coind->symbol, templ->version, templ->nbits, templ->ntime);
//	debuglog("coinb1 %s", templ->coinb1);
//	debuglog("coinb2 %s", templ->coinb2);
}
