...
static json_t *read_real32(buffer_t *buffer) {
	...
	return json_double_new((double)number);
}
...
static json_t *read_string(buffer_t *buffer) {
	...
	json_t *ret = json_string_new(str);
//	json_t *ret = json_string_new_nocopy(strlen(str),str);
	if (strlen(str)>0) 
			free(str);
	return ret;
}
...
static json_t *read_bytes(buffer_t *buffer) {
	...
	read_buffer(buffer, bytes, len*sizeof(unsigned char));
	json_value *val = json_bytes_new(bytes,len);
	jsonp_free(bytes);
	return val; //json_bytes_new_nocopy(len,bytes);  // check if it affects memory leaks
}

static json_t *read_array(buffer_t *buffer, json_error_t *error) {

	size_t len = read_uvarint(buffer);
	json_value *jarray = json_array_new(0);
...
static json_t *read_obj(buffer_t *buffer, json_error_t *error) {
	...
	for (unsigned int i = 0; i < len; ++i) {
		...
		json_object_push(object,key, entry);
		if (strlen(key)>0)
			free(key);
	}
...
//	buffer_t *buffer = jsonp_malloc(sizeof(buffer_t));
	...
	json_t *val = read_value(buffer, error);
	free(buffer);
	return val; //read_value(buffer, error);
}
